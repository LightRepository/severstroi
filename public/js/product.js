const products = [
    ...
    // Глобальные функции для закрытия модальных окон
    ...
    document.addEventListener('click', function(event) {
        ...
        if (event.target.matches('#closeOrderModalBtn, #closeOrderModalBtn *')) {
            closeOrderModal();
            return;
        }

        // Закрытие окна успеха
        if (event.target.matches('#closeSuccessModalBtn, #closeSuccessModalBtn *')) {
            closeSuccessModal();
            return;
        }

        // Кнопка "Продолжить покупки"
        if (event.target.matches('#continueShoppingBtn, #continueShoppingBtn *')) {
            closeSuccessModal();
            return;
        }
    ...
    // Назначаем обработчики для кнопок "Купить" через делегирование событий
    ...
    // Настройка кнопок "Купить" через делегирование событий
    function setupBuyButtonsDelegation() {
    document.addEventListener('click', function(event) {
        // Проверяем, была ли нажата кнопка "Купить" или её дочерний элемент
    ...
    if (customerName) {
    isValid = validateField(customerName, validateName) && isValid;
}

    if (customerPhone) {
    isValid = validateField(customerPhone, validatePhone) && isValid;
}

    if (customerEmail) {
    isValid = validateField(customerEmail, validateEmail) && isValid;
}
